...
public class VisitsController {
	...
	List<VisitDto> getVisits(@PathVariable long patientId,
			@RequestParam(required = false, name = "from") @Pattern(regexp = ISO_DATE, message = "wrong date format 'from'") String fromDate,
			@RequestParam(required = false, name = "to") @Pattern(regexp = ISO_DATE, message = "wrong date format 'to'") String toDate) {
