...
public class AvgValuesControllerTest {
	...
	void noDatesTest() throws UnsupportedEncodingException, Exception {
		String res = mockMvc.perform(get("/pulse/values/123")).andDo(print()).andExpect(status().isOk()).andReturn()
	...
	void dateFromTest() throws UnsupportedEncodingException, Exception {
		String res = mockMvc.perform(get("/pulse/values/123").param("from", "2023-01-01T12:00")).andDo(print())
	...
	void dateToTest() throws UnsupportedEncodingException, Exception {
		String res = mockMvc.perform(get("/pulse/values/123").param("to", "2023-01-01T12:00")).andDo(print())
				.andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
		assertEquals(AVG_VALUE_DATES + "", res);
	}
